...
// Controleert of de tegenstander nog een weg heeft naar het centrum.
export function opponentPathOpen(board, oppPos) {
  const visited = new Set([oppPos[0] + "," + oppPos[1]]);
  const queue = [oppPos];
  while (queue.length) {
...
export function applyPlaceTool(state, player, r, c) {
  ...
  if (!opponentPathOpen(trial, state.pawnPos[opp])) {
    return { ok: false, error: "Dit sluit de tegenstander volledig in." };
  }
...
export function applyMove(state, player, from, dir, endTurn) {
  ...
  const opp = otherPlayer(player);
  if (!winningMove && !opponentPathOpen(trial, state.pawnPos[opp])) {
    return { ok: false, error: "Deze zet blokkeert de tegenstander volledig." };
  }
