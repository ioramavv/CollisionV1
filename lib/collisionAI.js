...
// slide() in de engine, niet hemelsbrede afstand). Geeft zowel de kortste
// padlengte naar het centrum terug (Infinity als dat niet binnen
// MAX_BOUNCE_DEPTH stuiters lukt) als de volledige set bereikte
// landingsvakjes — dat laatste wordt hergebruikt om kandidaat-
// hulpstukplaatsingen te beperken tot vakjes die daadwerkelijk op een
// bereikbaar pad liggen (zie toolPlacementCandidates hieronder).
  ...
  while (frontier.length && depth < MAX_BOUNCE_DEPTH && pathLength === Infinity) {
    ...
    for (const p of frontier) {
      for (const dir of Object.keys(DIRS)) {
        const dest = slide(working, p, dir, isPawn);
        if (!dest) continue;
...
function toolPlacementCandidates(state, player) {
  ...
  const relevant = new Set([...oppExplore.visited, ...ownExplore.visited]);
  const results = [];
