...
// Weegfactor voor "kan deze pion het centrum bereiken via een stuiterpad
// vanaf hier" — domineert bewust de ruwe-afstand- en mobiliteitstermen
// hieronder, want een dreigende winst (van jezelf of de tegenstander) is
// veel belangrijker dan een paar vakjes dichter/verder bij het centrum.
const REACH_CENTER_WEIGHT = 50;
...
// Kan de pion op `pos` het centrum bereiken via een stuiterketen op dit
// bord? Dit is de échte winvoorwaarde (niet hemelsbrede afstand) en is dus
// de belangrijkste vraag om te stellen: staat iemand op het punt te winnen.
function centerReachable(board, pos) {
  return reachablePaths(board, pos, true).some(({ pos: p }) => isCenter(p[0], p[1]));
}
...
//
// De reach-center-termen zijn bewust dominant t.o.v. de ruwe afstand en
// mobiliteit: die laatste zijn alleen zinvolle tie-breakers zolang niemand
// een stuiterpad naar het centrum heeft. Zodra de tegenstander die wél
// heeft (en het bijna zijn beurt is), moet dat de zet-keuze overstemmen —
// anders "ziet" de bot een dreigende nederlaag niet, of mist hij zijn eigen
// winkans, ook al is dat met alleen hemelsbrede afstand niet te zien.
function evaluate(state, perspective) {
  ...
  const ownDist = centerDist(ownPos);
  const oppDist = centerDist(oppPos);
  ...
  if (centerReachable(state.board, ownPos)) score += REACH_CENTER_WEIGHT;
  if (centerReachable(state.board, oppPos)) score -= REACH_CENTER_WEIGHT;
  return score;
}
...
function reachablePaths(board, start, isPawn) {
  ...
  function dfs(pos, path) {
    if (path.length > 0) results.push({ pos, path });
...
function search(state, mover, botPerspective, depth, topK, alpha, beta) {
  ...
  // Score één keer per kandidaat berekenen (niet per vergelijking) — anders
  // roept .sort() evaluate() O(n log n) keer aan i.p.v. O(n), wat met de
  // reach-center-check in evaluate() flink oploopt.
...
export function chooseComputerTurn(state, bot, difficulty = "medium") {
  ...
  const scored = candidates.map((c) => ({ ...c, sortScore: evaluate(c.resultState, bot) }));
  scored.sort((a, b) => b.sortScore - a.sortScore);
  const rootCandidates = scored.slice(0, config.rootK);
