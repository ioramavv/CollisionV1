...
// boven: een ronde bolvormige voet, een getailleerde "nek" met kraaglijntjes,
// en een plat afgekante kopje — net als de echte gedraaide houten pionnen
// op het fysieke bord (zie de referentiefoto's).
function Pawn({ owner }) {
  const body = owner === "A" ? "collision-piece-maple-body" : "collision-piece-walnut-body";
  const top = owner === "A" ? "collision-piece-maple-top" : "collision-piece-walnut-top";
  return (
    <svg viewBox="0 0 100 150" style={{ width: "100%", height: "auto", display: "block" }} aria-hidden="true">
      <ellipse cx="50" cy="145" rx="27" ry="6" fill="rgba(0,0,0,0.32)" />
      {/* bolvormige voet — één vloeiende bocht, zonder extra deukjes */}
      <path
        d="M30,143 Q21,143 23,130 C27,116 34,106 34,90 L66,90
           C66,106 73,116 77,130 Q79,143 70,143 Z"
        fill={`url(#${body})`}
        stroke="rgba(0,0,0,0.26)"
        strokeWidth="1"
      />
      {/* nek — smallere taps toelopende hals naar het kopje */}
      <path
        d="M37,90 L34,58 C34,52 39,48 46,47 L54,47 C61,48 66,52 66,58 L63,90 Z"
        fill={`url(#${body})`}
        stroke="rgba(0,0,0,0.22)"
        strokeWidth="0.8"
      />
      <path d="M35,74 Q50,78 65,74" fill="none" stroke="rgba(0,0,0,0.22)" strokeWidth="1.4" />
      <path d="M35,68 Q50,72 65,68" fill="none" stroke="rgba(0,0,0,0.16)" strokeWidth="1.2" />
      {/* plat afgekant kopje, met een randje en houtnerf-ringetje bovenop */}
      <ellipse cx="50" cy="48" rx="17" ry="7" fill="rgba(0,0,0,0.12)" />
      <ellipse cx="50" cy="44" rx="16" ry="7" fill={`url(#${top})`} stroke="rgba(0,0,0,0.25)" strokeWidth="0.8" />
      <ellipse cx="50" cy="44" rx="8.5" ry="3.5" fill="none" stroke="rgba(0,0,0,0.18)" strokeWidth="0.8" />
    </svg>
  );
}

// Hulpstuk — een kort, breed kegeltje met een plat afgekante top, net als de
// kleine houten stukjes op de foto's van het fysieke bord (in tegenstelling
// tot de langere, getailleerde pion).
function ToolPiece({ owner }) {
  const body = owner === "A" ? "collision-piece-maple-body" : "collision-piece-walnut-body";
  const top = owner === "A" ? "collision-piece-maple-top" : "collision-piece-walnut-top";
  return (
    <svg viewBox="0 0 100 68" style={{ width: "100%", height: "auto", display: "block" }} aria-hidden="true">
      <ellipse cx="50" cy="63" rx="33" ry="6" fill="rgba(0,0,0,0.3)" />
      <path
        d="M19,60 Q14,60 16,54 C20,42 26,30 29,17 C29,13 32,10 37,8
           L63,8 C68,10 71,13 71,17 C74,30 80,42 84,54 Q86,60 81,60 Z"
        fill={`url(#${body})`}
        stroke="rgba(0,0,0,0.28)"
        strokeWidth="1"
      />
      <ellipse cx="50" cy="9" rx="18" ry="7" fill="rgba(0,0,0,0.12)" />
      <ellipse cx="50" cy="5" rx="17" ry="7" fill={`url(#${top})`} stroke="rgba(0,0,0,0.25)" strokeWidth="0.8" />
      <ellipse cx="50" cy="5" rx="9" ry="3.5" fill="none" stroke="rgba(0,0,0,0.2)" strokeWidth="0.8" />
    </svg>
  );
}
