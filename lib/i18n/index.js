...
// Supabase Auth-foutmeldingen komen altijd in het Engels terug, ongeacht de
// UI-taal — hier vangen we de meest voorkomende gevallen op (verkeerde
// inloggegevens, e-mail al in gebruik, te kort wachtwoord, rate limiting) en
// vertalen die naar de actieve taal. Onbekende/zeldzame Supabase-foutteksten
// vallen gewoon terug op de ruwe boodschap i.p.v. een vertaling te verzinnen.
export function translateAuthError(t, message) {
  if (!message) return message;
  const m = message.toLowerCase();
  if (m.includes("invalid login credentials")) return t("auth.error.invalidCredentials");
  if (m.includes("already registered")) return t("auth.error.emailTaken");
  if (m.includes("password should be at least") || m.includes("at least 6 characters")) return t("auth.error.weakPassword");
  if (m.includes("for security purposes") || m.includes("rate limit")) return t("auth.error.rateLimited");
  return message;
}
