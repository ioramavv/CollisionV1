  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4">
      <form onSubmit={handleSubmit} className="panel w-full max-w-sm flex flex-col gap-4">
        <h1 className="text-lg font-bold uppercase tracking-widest">{t("register.title")}</h1>
        <input className="input" placeholder={t("register.usernamePlaceholder")} value={username}
          onChange={(e) => setUsername(e.target.value)} required />
        <input className="input" type="email" placeholder={t("login.emailPlaceholder")} value={email}
          onChange={(e) => setEmail(e.target.value)} required />
        <input className="input" type="password" placeholder={t("register.passwordPlaceholder")} value={password}
          onChange={(e) => setPassword(e.target.value)} required minLength={6} />
        {error && <p className="text-sm" style={{ color: "#e07a5f" }}>{error}</p>}
        <button className="btn btn-solid" disabled={loading}>
          {loading ? t("common.busy") : t("register.title")}
        </button>
        <p className="text-xs mono" style={{ color: "var(--muted)" }}>
          {t("register.haveAccount")} <a href="/login" className="underline">{t("register.loginLink")}</a>
        </p>
      </form>
    </main>
  );
